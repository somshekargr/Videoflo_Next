...
export default function TestCheckListNew(props) {
  //Redux Subscriptions
  ...
  const onCheckListCompleted = () => {
    if (activeCheckListIndex < customCheckList.length - 1) {
      ...
      websocket.emit(WorkflowEvents.onPrecallCustomChecklistCompleted, {
...
const CustomCheckList = (props) => {
  ...
  return (
    <>
      ...
      {checkListCompleted &&  <Button
          onClick={() => props.onCheckListCompleted()}
          disabled={!checkListCompleted}
          className={checkListCompleted ? DcStyle.ebtnStyle : DcStyle.dbtnStyle}
          variant="body2"
        >
